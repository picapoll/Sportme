...
*https://code.google.com/archive/p/geoxml-v3/                                 *
\******************************************************************************/
// Constructor
...
function GeoXml (myvar, map, url, opts) {
  ...
  if (this.opts.singleInfoWindow) {
    google.maps.event.addListener(map, 'click', function () {
		  if (this.geoxml.lastMarker.infoWindow.getMap())		  	{ this.geoxml.lastMarker.infoWindow.close() }
...
GeoXml.prototype.createMarkerJSON = function (item, idx) {
  ...
  if (item.shadow) { style.shadow = item.shadow } else { style.shadow = null }
...
GeoXml.prototype.createMarker = function (point, name, desc, styleid, idx, instyle, visible, kml_id, markerurl, snip) {
  ...
  if (start.match(/^http/i) || start.substr(0, 1) == '/') {
    ...
    if (typeof this.url === 'string') {
      ...
      if (newurl == '') { markeroptions.icon.url = icon.url } else { markeroptions.icon.url = newurl + '/' + icon.url }
    }
  }
  ...
  if (name != desc) {
    ...
    // Infowindow-opening event handler
      google.maps.event.addListener(m, this.iwmethod, function () {
        ...
        if (!m.infoWindow.getMap()) { m.infoWindow.open(m.geoxml.map, this) }
      })
    }
	    }
  if (this.extcontentmarkers) {
      ...
      if (m.contentUrl.length > 0) {
        ...
        for (var x = 0; x < m.contentUrl.length; x++) {
          ...
          setTimeout(m.geoxml.myvar + ".getextContent('" + m.contentUrl[x] + "', " + x + ')', 100 + x * 50)
			  ...
			  } else					{ m.infoWindow.open(m.geoxml.map, this) }
    })
  }
  if (this.contentlinkmarkers) {
    google.maps.event.addListener(m, this.linkmethod, function () {
...
GeoXml.prototype.createPolyline = function (lines, color, width, opacity, pbounds, name, desc, idx, visible, kml_id) {
  ...
  var isnew = true
  var p = {}
  if (!color) { p.color = that.randomColor() } else { p.color = color }
  if (!opacity) { p.opacity = 0.45 } else { p.opacity = opacity }
  if (!width) { p.width = 4 }		 else { p.width = width }
  p.idx = idx
...
GeoXml.prototype.finishPolygonJSON = function (op, idx, updatebound, lastpoly) {
  ...
  if (!op.fill) { p.fillOpacity = 0.0 } else	{ p.fillOpacity = op.opacity }
...
GeoXml.prototype.recurseJSON = function (doc, title, desc, sbid, depth) {
  ...
  if ((!depth && (doc.folders && doc.folders.length > 1)) || doc.marks.length) {
    if (depth < 2 || doc.marks.length < 1) { icon = that.globalicon } else { icon = that.foldericon }
...
GeoXml.prototype.createPolygon = function (lines, color, width, opacity, fillcolor, fillOpacity, pbounds, name, desc, folderid, visible, fill, outline, kml_id) {
  ...
  if (!color) { p.obj.strokeColor = this.style.color } else { p.obj.strokeColor = color }

  if (!fillcolor) { p.obj.color = this.randomColor() } else { p.obj.color = fillcolor }

  if (opacity) { p.obj.opacity = opacity } else {
    p.obj.opacity = this.style.opacity
    p.obj.strokeOpacity = this.style.opacity
  }

  if (fillOpacity) { p.obj.fillOpacity = fillOpacity } else {
	   p.obj.fillOpacity = this.style.fillopacity
  }

  if (!width) { p.obj.strokeWeight = this.style.width } else { p.obj.strokeWeight = width }
...
GeoXml.prototype.handleGeomark = function (mark, idx, trans) {
  ...
  if (coordset.length < 1) {
    ...
    for (l = 0; l < poslist.length; l++) {
      ...
      if (coords) {
 				if (poslist.item(l).parentNode && (poslist.item(l).parentNode.nodeName == 'gml:LineString')) { line_count++ } else { poly_count++ }
...
GeoXml.prototype.handlePlacemarkGeometry = function (mark, geom, idx, depth, fullstyle) {
  ...
  for (var ln = 0; ln < mark.childNodes.length; ln++) {
    ...
    var ns = nn.split(':')
    var base
    if (ns.length > 1) { base = ns[1].toLowerCase() } else { base = ns[0].toLowerCase() }
...
google.maps.Polyline.prototype.getBounds = function () {
  if (typeof this.bounds !== 'undefined') { return this.bounds } else { return (this.computeBounds()) }
}
...
//			}
  ...
  for (i = 0; i < overlaymanager.markers.length; ++i) {
    ...
    if (marker !== null) {
      ...
      if (Lance$(mid)) {
        if (vis) { Lance$(mid).className = 'inView' }						 else { Lance$(mid).className = 'outView' }
      }
				// alert(vis);
  ...
  if (newzoom) {
    if (viscount > overlaymanager.maxVisibleMarkers) { overlaymanager.cluster.setMinimumClusterSize(overlaymanager.minMarkersPerCluster) } else			{ overlaymanager.cluster.setMinimumClusterSize(overlaymanager.maxVisibleMarkers) }
...
OverlayManager.PopUp = function (overlaymanager, cClusterIcon) {
  for (x = 0; x < overlaymanager.cluster.clusters_.length; x++) {
    if (cClusterIcon == overlaymanager.cluster.clusters_[x].clusterIcon_) { break }
  }
  ...
  if (overlaymanager.paren.lastMarker && overlaymanager.paren.lastMarker.infoWindow) { overlaymanager.paren.lastMarker.infoWindow.close() }
  var infoWindowOptions = {
    content: html,
    pixelOffset: new google.maps.Size(0, 2),
...
OverlayManager.prototype.getMapBounds = function (overlaymanager) {
  ...
  if (overlaymanager.map.getZoom() > 1) {
	  ...
	  if (typeof b === 'undefined')	 	{ bounds = new google.maps.LatLngBounds(new google.maps.LatLng(-85.08136444384544, -178.48388434375), new google.maps.LatLng(85.02070771743472, 178.00048865625)) } else		{ bounds = new google.maps.LatLngBounds(b.getSouthWest(), b.getNorthEast()) }
  } else {
...
Cluster.prototype.remove = function () {
  if (this.clusterIcon_.infoWindow) { this.clusterIcon_.infoWindow.close() }
